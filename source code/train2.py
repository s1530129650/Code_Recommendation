...
for epoch in range(num_epochs):
    ...
    for i, data4 in enumerate(data_loader,0):
        ...
        input_len, input1,input2, target, parent = zip(*Z)
        ...
        '''
        input = [data4[0].to(device),data4[1].to(device)]
        input_len = data4[2]
        target = data4[3].to(device)
        parent = data4[4]
    ...
    epoch + 1, (now - time_start)/60, total_loss / len_data))
    losses.append(total_loss / len_data)
