        ...
        if test_data.ctx[-1] == key:  # secondDict[key]＝N
            ...
            if type(secondDict[key]).__name__ == 'dict':  # type(secondDict[key]).__name__＝str
                ...
                if len(claf) <3 :
                    ...
                    top_k = 5
                    suggestion = result.argsort()[::-1][0:top_k ]
                    #print("suggestion result1", claf[2].classes_,suggestion)
                    suggestion = list(claf[2].inverse_transform(suggestion))
                    #print("suggestion result2", suggestion)
            break
...
def main_tree():
    ...
    quer_data = gene_queries()[:20000]

    now = time.time()
    print("1 data load",now- start_time)
    random.shuffle(quer_data) #shuffle
    trn_data = quer_data[:10000]
    test_data = quer_data[10000:20000]
    ...
    print("data set size",len(training_data),len(eval_data))
    ...
    with open('model_100w.pickle', 'wb') as f1:
