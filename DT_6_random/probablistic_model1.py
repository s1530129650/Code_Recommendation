...
def Uncond_probab_model(labelCounts, leng):
...
def RandomCache():
    pos_label = numpy.random.randint(low = 0, high=100, size=10000)
    len_label = 10000
    LabelPosCounts = Counter(pos_label )
    return  Uncond_probab_model(LabelPosCounts, len_label)
    ...
    for key in secondDict.keys():
        if type(secondDict[key]).__name__ == 'dict':  # type(secondDict[key]).__name__＝str
            ...
            for data in leaf_data:
                ...
                train_pos_label.append(data.position)
            #print("train_pos_label",train_pos_label)
            # 3 data: text -> num ->vect
            #print(len(leaf_data),train_data)
            labelCounts = Counter(train_label)
            len_label = len(train_label)
            LabelPosCounts = Counter(train_pos_label)
            # DT+ model
            # cache model
            cache_model = RandomCache()
            if len(labelCounts) <= 5:
                secondDict[key] = [Uncond_probab_model(labelCounts, len_label ),cache_model]
                continue
            ...
            secondDict[key] = [p_best_feat,Mult_NB(train_vec, labels),le,cache_model]
