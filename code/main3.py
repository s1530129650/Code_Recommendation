...
"""
# 合并了之前的model为一个model ，增加了vocabulary的length limit
import torch
...
for i in range(len(training_data)):
    print
    rd = random.randint(1, len(training_data[i]) - 1) #取rand int rd
    length = len(training_data[i])
    '''
    while "empty" in training_data[i][rd].values(): #remove value不为empty的node
        rd = rd + 1
        rd = rd % length
    '''
...
class Context_atten(nn.Module):
    ...
    def forward(self, inputs, hc):
        ...
        '''
        if inputs[0] <= context_window:
            Mt = inputs[-self.context_window:, :, :] # context size 实则为少一个，because size of some sequence is less than context size
        else:
            Mt = inputs[-self.context_window - 1:-1, :, :]
        '''
        Mt = inputs[-self.context_window:,:,:]  # context size 实则为少一个，because size of some sequence is less than context size
