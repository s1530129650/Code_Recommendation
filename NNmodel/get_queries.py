...
def Queries(Data):
    ...
    for data in Data:
        length = len(data)
        if length <=  CONTEXT_WINDOW+1  or length > 2000:
            continue
...
test_queries = test_queries[:100000]
...
def textToindex(prepare_sequence,queries,value_vocab,type_vocab):
    ...
    parent = []
    target = []
    #query = [0 context,   1 predict_node,  2 position, 3 same_node_position, 4 parent_node_position]
