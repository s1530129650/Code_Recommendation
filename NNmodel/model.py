...
class Context_atten(nn.Module):
    ...
    def forward(self, inputs,setence_len, hc,parent):
        ...
        for i in range(batch_size - 1):
            Mt = torch.cat([Mt, inputs[i + 1, setence_len[i + 1] - timestep : setence_len[i + 1], :].unsqueeze(0)], 0) #[B,T,H]
...
class Parent_atten(nn.Module):
    ...
    def forward(self, ht, ct, pt):
        ...
        Gt = torch.tanh(self.Wg_linear(torch.cat([ht,ct, pt], 2)))# (1,B,H)
...
class main_model(nn.Module):
    def __init__(self, vocab_value_size, value_dim, vocab_type_size, type_dim,
        ...
        self.h0 = nn.Parameter(torch.rand(1, self.batch_size, self.hidden_dim))
        self.c0 = nn.Parameter(torch.rand(1, self.batch_size, self.hidden_dim))
    ...
    def forward(self, inputs,input_len,parent):
        output, hn, cn = self.model_LSTM(inputs,input_len,(self.h0,self.c0))
