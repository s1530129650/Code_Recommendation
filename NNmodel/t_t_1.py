...
with open('../data/python/train3_50k.pickle', 'rb') as f:
...
with open('../data/python/eval3_50k.pickle', 'rb') as f:
...
with open('../data/python/test3_50k.pickle', 'rb') as f:
    data_test = pickle.load(f)
len_test = data_test.length
print("len_test",len_test)
data_loader_test = DataLoader(data_test, batch_size= BATCH_SIZE, shuffle=True, drop_last=True)
...
def eval(model,eval_data,loss_function):
    ...
    for i, data4 in enumerate(eval_data,0):
       ...
       # print("target",target)
        ...
        #print(i, "batch time spend", end - start)
    print("A_P",len(A_P),len_eval)
    m_a_p = sum(A_P) / len(A_P)
